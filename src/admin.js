...
export async function renderAdmin(nombre, onLogout) {
    ...
    <div style="max-width:1100px;margin:0 auto;padding:24px;display:grid;gap:20px">
      ...
      <div style="background:#fff;border-radius:12px;border:1px solid #E5E7EB;padding:22px">
        ...
        <div style="overflow-x:auto">
          <table id="a-hist-tabla" class="hidden" style="width:100%;border-collapse:collapse;font-size:13px;min-width:720px">
            <thead><tr style="border-bottom:1px solid #E5E7EB">
  ...
  function kpiCard(label, value, sub, color = '#1F2937') {
  ...
  function renderKPIs() {
    ...
    const gast = gastos.reduce((s, g) => s + Number(g.monto), 0)
    const neta = util - gast
  ...
  function renderGastos() {
    const lista = el.querySelector('#a-gastos-lista')
    if (!gastos.length) {
      lista.innerHTML = '<div style="text-align:center;padding:20px;color:#9CA3AF;font-size:13px">Sin gastos registrados</div>'
      return
    }
    lista.innerHTML = gastos.slice(0, 10).map(g => `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #F3F4F6">
        ...
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <button onclick="window._delGasto(${g.id})"
            style="padding:2px 8px;background:#FCEBEB;color:#A32D2D;border:none;border-radius:6px;font-size:12px;cursor:pointer">×</button>
        </div>
      </div>`).join('')
  }
  ...
  el.querySelector('#a-guardar-precios').addEventListener('click', async () => {
    ...
    try {
      ...
      toast('Precios actualizados — el vendedor los ve al instante')
    } catch(e) { toast('Error al guardar precios', true) }
  ...
  el.querySelector('#g-registrar').addEventListener('click', async () => {
    ...
    try {
      ...
      renderTodo(); toast('Gasto registrado')
    } catch(e) { toast('Error al registrar gasto', true) }
  })

  el.querySelector('#a-exportar').addEventListener('click', () => {
    if (!ventas.length) { toast('Sin ventas para exportar', true); return }
    const rows = [
      'Fecha,Vendedor,Turno,Producto,Monto,Gramos,Utilidad,Margen%',
      ...ventas.map(v =>
        `${v.fecha},${v.vendedor},${v.turno},${v.producto_nombre},${Number(v.monto).toFixed(2)},${Number(v.gramos).toFixed(1)},${Number(v.utilidad).toFixed(2)},${(Number(v.margen)*100).toFixed(1)}`)
    ]
    const blob = new Blob([rows.join('\n')], { type: 'text/csv' })
    const a = document.createElement('a')
    a.href = URL.createObjectURL(blob)
    a.download = `graneles_${filtroDesde}_${filtroHasta}.csv`
  ...
  window._precioChange = (input) => {
    ...
    if (!prod) return
    if (tipo === 'precio') prod.precio_lb = val
    else prod.costo_lb = val
